...
pub fn score(data: &[u8]) -> i64 {
    ...
    while i < data.len() {
        ...
        // Thumb
        if half == 0x4770 { score -= 15; i += 2; continue; }  // BX LR
        if half == 0xBF00 { score -= 10; i += 2; continue; }  // NOP
        if matches!(half, 0xB672 | 0xB662 | 0xB673 | 0xB663) { score -= 12; i += 2; continue; } // CPSID/CPSIE
        if (half & 0xFF00) == 0xB500 { score -= 8; i += 2; continue; }  // PUSH {.., LR}
        if (half & 0xFF00) == 0xBD00 { score -= 8; i += 2; continue; }  // POP {.., PC}
        // AVR
        if half == 0x9508 { score -= 12; i += 2; continue; }  // RET
        if half == 0x9518 { score -= 10; i += 2; continue; }  // RETI
        // MSP430
        if half == 0x4130 { score -= 12; i += 2; continue; }  // RET
        if half == 0x4303 { score -= 8; i += 2; continue; }   // NOP
        ...
        if major >= MAJOR_16BIT_START && major <= MAJOR_16BIT_END {
            ...
            // AArch64
            if word == 0xD65F03C0 { score -= 15; i += 4; continue; }  // RET
            if word == 0xD503201F { score -= 10; i += 4; continue; }  // NOP
            if (word >> 26) == 0x25 { score -= 5; }                   // BL
            // RISC-V
            if word == 0x00008067 { score -= 12; i += 4; continue; }  // RET
            if word == 0x00000013 { score -= 8; i += 4; continue; }   // NOP
            // Thumb-2
            {
                let hw_low = (word & 0xFFFF) as u16;
                let hw_high = (word >> 16) as u16;
                if hw_low == 0xE92D { score -= 10; i += 4; continue; }  // PUSH.W
                if hw_low == 0xE8BD { score -= 10; i += 4; continue; }  // POP.W
                if (hw_low & 0xF800) == 0xF000 && (hw_high & 0xD000) == 0xD000 {
                    score -= 8; i += 4; continue;  // BL
                }
            }
            // ARM32
            if word == 0xE12FFF1E { score -= 15; i += 4; continue; }  // BX LR
            if word == 0xE1A00000 { score -= 12; i += 4; continue; }  // NOP
            ...
            if word == ARC_NOP_32_LE || word == ARC_NOP_32_BE {
                ...
                match major32 {
                    0x04 | 0x05 => score += 3,
                    0x01 | 0x00 => { score += 4; branch_count += 1; }
                    0x02 | 0x03 => score += 3,
                    _ => { score -= 1; }  // Unrecognized 32-bit
                }
            }
